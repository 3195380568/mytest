...
public class MyController {
    ...
    public void load(String filename,HttpServletRequest request, HttpServletResponse response) throws IOException {
        response.setHeader("Content-Disposition","attachment;filename="+doFilename2(filename,request));
        String realPath = request.getServletContext().getRealPath("/images");
        System.out.println("下载路径="+realPath);
        FileOutputStream is = new FileInputStream(new File(realPath,filename));
